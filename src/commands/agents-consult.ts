...
export async function agentsConsultCommand(args: string, ctx: ExtensionCommandContext): Promise<void> {
  const { agentId, message } = parseAgentMessage(args);
  if (!agentId || !message) {
    ctx.ui.notify("Usage: /agents-consult <worktree-agent-id> <question>", "warning");
    return;
  }
  const result = await messageParallelAgent({ agentId, mode: "consult", message }, ctx);
